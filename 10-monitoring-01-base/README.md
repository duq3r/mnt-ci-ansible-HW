...
RAM - количество ичпользуемой оперативнйо памяти <br>
inodes -  количество  использованных индексных дескрипторов , количество которых ограниченно <br>
...
Идея, что портят метрику только 500-е и 400е коды. Их отсутствие, т.е. все 200е = 100% отнимаем процент плохих кодов и получаем метрику SLI (Реальные цифры доступности нашего продукта). <br>
```
SLI = 100 - ((summ_5xx_requests + summ_4xx_requests)/(summ_all_requests)*100)
```  